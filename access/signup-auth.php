...
$site_url = "http://bolttrades.com/dashboard/";
$site_reset = "http://bolttrades.com/";
if ($_GET['action'] == "signup") {
	...
	//var_dump($msg);
	$qin = mysqli_query($db, "insert into users (name,last_name,email,password,vcode,isVerified,referal,referal_link,image) values ('$fname','$lname','$email','$password','$vcode',0,'$referal','$referal_link','uploads/user.png')");
	if ($qin) {
		//send mail
		...
		$result = Mail::sendMail($email, "Welcome ".$nane, $msg);
...
// 		}
	...
	$rand = rand(1000, 9999);
	$chk1 = mysqli_query($db, "select * from users where email='$email' and password='$password' LIMIT 1");
	$row = mysqli_fetch_array($chk1);
	if (!$row) {
	...
	$result = Mail::sendMail('admin@neocapitals.com', "Enquiry", $msg);
	if ($result == "success") {
	...
	$msg .= '<a href="https://neocapitals.com/access/admin.php">Click to confirm</a><br/>';

	$result = Mail::sendMail('admin@neocapitals.com', "Deposit", $msg);
	if ($result == "success") {
	...
	$result = Mail::sendMail('admin@neocapitals.com', "Farm Deposit", $msg);
	if ($result == "success") {
		echo json_encode(["status" => true, "message" => "Confirmation request is sent.. We will confirm and approve payment shorlty"]);
		return;
	} else {
